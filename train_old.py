...
class DiceBCELoss(nn.Module):
    ...
    def forward(self, logits, targets):
        ...
        dice_loss = 1. - dice_coeff(logits, targets)
...
def train(model, trainloader, valloader, optimizer, loss,model_file, epochs,save_checkpoint=True, best_l_dice = 0,patience = 10):
    ...
    for epoch in tqdm(range(epochs)):
        ...
        is_best = val_l_dice > best_l_dice
        if val_l_dice > best_l_dice:
            best_l_dice = val_l_dice
            patience = patience
        else:
            patience -=1
